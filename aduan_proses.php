...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aduanuser'])) {
    ...
    if (!empty($kategori) && !empty($jenis_aset) && !empty($no_siri) && !empty($tempat_rosak) && !empty($userterakhir) && !empty($ulasan) && !empty($nama) && !empty($tarikh_rosak) && !empty($emel)) {
        // Default value for 'lulus_jabatan'
        $lulus_jabatan = 'Dalam Proses';

        // Prepared statement to prevent SQL injection
        $stmt = $conn->prepare("INSERT INTO tbl_semakan (kategori, jenis_aset, no_siri, tempat_rosak, userterakhir, ulasan, nama, emel, tarikh_rosak, lulus_jabatan) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssss", $kategori, $jenis_aset, $no_siri, $tempat_rosak, $userterakhir, $ulasan, $nama, $emel, $tarikh_rosak, $lulus_jabatan);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aduanadmin'])) {
    // Validate all inputs
    $kategori = isset($_POST['kategori']) ? trim($_POST['kategori']) : '';
    $jenis_aset = isset($_POST['jenis_aset']) ? trim($_POST['jenis_aset']) : '';
    $no_siri = isset($_POST['nomborsiri']) ? trim($_POST['nomborsiri']) : '';
    ...
    if (!empty($kategori) && !empty($jenis_aset) && !empty($no_siri) && !empty($tempat_rosak) && !empty($userterakhir) && !empty($ulasan) && !empty($nama) && !empty($tarikh_rosak) && !empty($emel)) {
        ...
        if ($stmt->execute()) {
            ...
            try {
                ...
                <script>
                alert('Aduan berjaya dihantar dan Email telah dihantar!');
                document.location.href = 'aduan_admin.php';
                </script>
                ";
            } catch (Exception $e) {
                echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
